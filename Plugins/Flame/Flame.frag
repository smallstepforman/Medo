...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    vec2 uv = vTexCoord0;//fragCoord.xy / iResolution.xy;
    ...
    color += (texture(iChannel0,uv).xyz*0.01*pow((r.y+r.y)*.65,5.0)+0.055)*mix( vec3(.9,.4,.3),vec3(.7,.5,.2), uv.y);
    color = mix(color, texture(iChannel0,uv).bgr, uMix);
    color = color/(uBrightness+max(vec3(0),color));
    fragColor = vec4(color.b, color.g, color.r, alpha);
}

void main()
 {
	mainImage(fFragColour, vec2(gl_FragCoord.x, iResolution.y - gl_FragCoord.y));
}
